...
{
    if(use_udp_)
        ROS_INFO_STREAM("Connecting to UDP data channel at hostname: " << conn_info.GetAddress()
                        << " tcp_port:" << conn_info.GetPort());
    else
        ROS_INFO_STREAM("Connecting to TCP data channel at hostname: " << conn_info.GetAddress()
                        << " tcp_port:" << conn_info.GetPort());

    if (Connect())
        ROS_INFO_STREAM("Lidar connect success.");
    else
        ROS_ERROR_STREAM("Lidar connect failed.");
...
bool LaserDataReceiver::Disconnect()
{
    ...
    try {
        ...
        // is_connected_ = false;
        ROS_INFO_STREAM("Disconnect succeed");
        return true;
        
    } catch (std::exception& e) {
        ROS_INFO_STREAM("Exception:" << e.what());
        ROS_WARN_STREAM("Disconnect fail");
        return false;
    }    
}
...
bool LaserDataReceiver::CheckConnection()
{
    ...
    {
        ROS_ERROR_STREAM("disconnect over time, timeout:" << (now_time - last_data_time_)
                         << " now:" << now_time
...
ScanData LaserDataReceiver::GetFullScan()
{
  ...
  if( scan_data_.size() >= 2 && IsConnected())
  {
    ...
    ROS_WARN_STREAM_THROTTLE(2, "null data");
...
void LaserDataReceiver::HandleSocketRead(const boost::system::error_code& error)
{
    ...
    //      udp_socket_ptr_->async_wait(boost::asio::ip::udp::socket::wait_read, boost::bind(&LaserDataReceiver::HandleSocketRead, this, boost::asio::placeholders::error));
        ...
        ROS_ERROR_STREAM("HandleSocketRead ---- ERROR");
        if (error.value() != 995)
            ROS_ERROR_STREAM("ERROR: data connection error:"
                    << error.message()
                    << " "
                    << error.value());
        Disconnect();
...
int LaserDataReceiver::SyncWrite()
{
  ...
  if(ec)
  {
    ROS_ERROR_STREAM("write kStartCapture failed:%" << boost::system::system_error(ec).what());
    ret = -1;
  }
  // ROS_INFO_STREAM("SyncWrite0 done");
  #ifdef DEBUG
    // ROS_WARN_STREAM("SyncWrite kStartCapture done");
  #endif
  return ret;
}
...
int LaserDataReceiver::SyncWrite(const std::array<unsigned char, 12> command)
{
  ...
  if(ec)
  {
    ROS_ERROR_STREAM("write 12command failed:" << boost::system::system_error(ec).what());
    ret = -1;
  }
  #ifdef DEBUG
  std::cout << "SyncWrite12 command done" << std::endl;
  #endif
  // ROS_WARN_STREAM("SyncWrite12 done");
  return ret;
}
...
int LaserDataReceiver::SyncWrite(const std::array<char, 18> command)
{
  ...
  if(ec)
  {
    ROS_ERROR_STREAM("write 18command failed:" << boost::system::system_error(ec).what());
    ret = -1;
  }
  #ifdef DEBUG
  std::cout << "SyncWrite18 command done" << std::endl;
  #endif
  // ROS_WARN_STREAM("SyncWrite done");
  return ret;
}
...
std::array<unsigned char, 12> LaserDataReceiver::GenerateParamCommand(XingSongLaserParam param)
{
  ...
  {
    set_param_command.at(7) = 0x00;
    ROS_WARN_STREAM("measure_frequency_KHz error!");
  }
...
bool LaserDataReceiver::GetRangeData(int16_t head_index)
{
  ...
  if(ring_buffer_.size() - head_index >= kXSPackageHeadSize)
  {
    ...
    if((ring_buffer_.size() - head_index) >= body_size)
    {
      ...
      // 计算本帧第一点的索引值
      begin_point_index = (header.start_angle-20)/angle_increment_+header.data_position-header.data_size;
...
void LaserDataReceiver::Connect_handler(const boost::system::error_code& error)
{
  ...
  {
    is_connected_ = true;
    ROS_INFO("async_connect succeed");
  }
  else
  {
    is_connected_ = false;
    ROS_ERROR_STREAM("Exception:" << error);
    ROS_ERROR("async_connect fail");
  }
}
}
