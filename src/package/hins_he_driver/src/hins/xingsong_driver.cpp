...
void XingSongDriver::DisturbFilter(ScanData& scan_data, DisturbFilterParam disturb_param)
{
  ...
  {
    scan_data.distance_data[disturb_del_vector[i]] = kMaxDistance + 1000;
  }
...
void XingSongDriver::RunMain()
{
  ...
  {
    ROS_INFO_STREAM("laser connect success.");
  }
  else
  {
    ROS_ERROR_STREAM("laser connect failed, please check the connection.");
  }
  ...
  while(ros::ok()) // @todo: stop loop condition need to modified
  {
    ...
    {
      while(!IsConnected()){
        ROS_WARN_STREAM("laser disconnected, trying to reconnect... ");
        if(StartCapturingTCP()){
          ROS_INFO_STREAM("laser reconnect success.");
        }
        usleep(1000 * 50); // 50ms
      }
    }
