...
bool LaserMapping::LoadParams(ros::NodeHandle &nh) {
    ...
    // add 1218
    nh.param<std::string>("publish/odometry_topic", odometry_topic_, "/Odometry");
    nh.param<std::string>("publish/path_topic", path_topic_, "/path");
...
bool LaserMapping::LoadParamsFromYAML(const std::string &yaml_file) {
    ...
    try {
        ...
        tf_imu_frame_ = yaml["publish"]["tf_imu_frame"].as<std::string>(tf_imu_frame_);
        tf_world_frame_ = yaml["publish"]["tf_world_frame"].as<std::string>(tf_world_frame_);
...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    pub_odom_aft_mapped_ = nh.advertise<nav_msgs::Odometry>(odometry_topic_, 100000);
    pub_path_ = nh.advertise<nav_msgs::Path>(path_topic_, 100000);
}
