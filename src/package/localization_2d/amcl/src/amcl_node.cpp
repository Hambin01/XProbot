...
private:
  ...
  geometry_msgs::PoseWithCovarianceStamped init_pose_last;
};
...
void AmclNode::savePoseToServer()
{
  ...
  init_pose_last.pose.pose.position.x = map_pose.getOrigin().x();
  init_pose_last.pose.pose.position.y = map_pose.getOrigin().y();
  init_pose_last.pose.pose.orientation.x = map_pose.getRotation().x();
  init_pose_last.pose.pose.orientation.y = map_pose.getRotation().y();
  init_pose_last.pose.pose.orientation.z = map_pose.getRotation().z();
  init_pose_last.pose.pose.orientation.w = map_pose.getRotation().w();

  ROS_DEBUG("Saving pose to server. x: %.3f, y: %.3f", map_pose.getOrigin().x(), map_pose.getOrigin().y());

  if (!std::isnan(map_pose.getOrigin().x()))
    private_nh_.setParam("initial_pose_x", map_pose.getOrigin().x());
  if (!std::isnan(map_pose.getOrigin().y()))
    private_nh_.setParam("initial_pose_y", map_pose.getOrigin().y());
  if (!std::isnan(yaw))
    private_nh_.setParam("initial_pose_a", yaw);
  if (!std::isnan(last_published_pose.pose.covariance[6 * 0 + 0]))
    private_nh_.setParam("initial_cov_xx", last_published_pose.pose.covariance[6 * 0 + 0]);
  if (!std::isnan(last_published_pose.pose.covariance[6 * 1 + 1]))
    private_nh_.setParam("initial_cov_yy", last_published_pose.pose.covariance[6 * 1 + 1]);
  if (!std::isnan(last_published_pose.pose.covariance[6 * 5 + 5]))
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan)
{
  ...
  if (resampled || force_publication)
  {
    ...
    if (max_weight > 0.0)
    {
       ...
       */
      if (std::isnan(hyps[max_weight_hyp].pf_pose_mean.v[0]) || std::isnan(hyps[max_weight_hyp].pf_pose_mean.v[1]) || std::isnan(hyps[max_weight_hyp].pf_pose_mean.v[2]))
      {

        init_pose_last.header.frame_id == "map";
        handleInitialPoseMessage(init_pose_last);
        if (force_update_after_initialpose_)
        {
          m_force_update = true;
        }
        ROS_WARN("some error,init again!");
      }
      else
        pose_pub_.publish(p);
...
void AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped &msg)
{
  ...
  if (!std::isnan(pose_new.getOrigin().x()) && !std::isnan(pose_new.getOrigin().y()) && !std::isnan(tf2::getYaw(pose_new.getRotation())))
  {
